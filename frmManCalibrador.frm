...
Begin VB.Form frmManCalibrador 
   ...
   ClientTop       =   330
   ClientWidth     =   13350
   Icon            =   "frmManCalibrador.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5925
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   6
      TabIndex        =   0
      Tag             =   "Código Variedad|N|N|0|999999|rcalidad_calibrador|codvarie|000000|S|"
      Text            =   "Var"
      Top             =   4950
      Width           =   800
   End
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmManCalibrador.frx":000C
      Height          =   4410
      Left            =   135
      TabIndex        =   10
      Top             =   540
   ...
   End
End
Attribute VB_Name = "frmManCalibrador"
...
'    chkVistaPrevia.Value = CheckValueLeer(Name)
    ...
    '****************** canviar la consulta *********************************+
    CadenaConsulta = "SELECT rcalidad_calibrador.codvarie, variedades.nomvarie, rcalidad_calibrador.codcalid, rcalidad.nomcalid, "
    CadenaConsulta = CadenaConsulta & "rcalidad_calibrador.numlinea, rcalidad_calibrador.nomcalibrador1, "
    CadenaConsulta = CadenaConsulta & " rcalidad_calibrador.nomcalibrador2, rcalidad_calibrador.nomcalibrador3 "
    CadenaConsulta = CadenaConsulta & " FROM variedades, rcalidad, rcalidad_calibrador "
    CadenaConsulta = CadenaConsulta & " WHERE variedades.codvarie = rcalidad.codvarie and "
    CadenaConsulta = CadenaConsulta & " rcalidad.codvarie = rcalidad_calibrador.codvarie and "
    CadenaConsulta = CadenaConsulta & " rcalidad.codcalid = rcalidad_calibrador.codcalid "
    '************************************************************************
...
'    adodc1.ConnectionString = Conn
    ...
    If ParamVariedad <> "" Then SQL = SQL & " and rcalidad_calibrador.codvarie = " & ParamVariedad
    
    
    '********************* canviar el ORDER BY *********************++
    SQL = SQL & " ORDER BY rcalidad_calibrador.codvarie, rcalidad_calibrador.codcalid, rcalidad_calibrador.numlinea"
    '**************************************************************++
    
    CargaGridGnral Me.DataGrid1, Me.adodc1, SQL, PrimeraVez
    
    ' *******************canviar els noms i si fa falta la cantitat********************
    tots = "S|txtAux(0)|T|Variedad|800|;S|btnBuscar(0)|B|||;S|txtAux2(2)|T|Denominación|2100|;"
    tots = tots & "S|txtAux(1)|T|Calidad|700|;S|btnBuscar(1)|B|||;S|txtAux2(1)|T|Denominación|2100|;"
    tots = tots & "S|txtAux(2)|T|Linea|800|;"
    ...
    Else
        tots = tots & "S|txtAux(3)|T|Precalibradora|2000|;S|txtAux(4)|T|Escandalladora|2000|;"
        tots = tots & "S|txtAux(5)|T|Calibrador Kaki|2000|;"
    End If
...
Private Sub txtAux_LostFocus(Index As Integer)
    ...
    Select Case Index
        ...
        Case 1 'codigo de calibre
            ...
            txtAux2(1).Text = DevuelveDesdeBDNew(cAgro, "rcalidad", "nomcalid", "codvarie", txtAux(0).Text, "N", , "codcalid", txtAux(1).Text, "N")
...
Dim Mens As String
    ...
    If b And (Modo = 3 Or Modo = 4) Then
        SQL = "select count(*) from rcalidad_calibrador where codvarie = " & DBSet(txtAux(0).Text, "N")
        SQL = SQL & " and codcalid <> " & DBSet(txtAux(1).Text, "N")
        SQL = SQL & " and nomcalibrador1 = " & DBSet(txtAux(3).Text, "T")
    ...
    If b And (Modo = 3 Or Modo = 4) Then
        SQL = "select count(*) from rcalidad_calibrador where codvarie = " & DBSet(txtAux(0).Text, "N")
        SQL = SQL & " and codcalid <> " & DBSet(txtAux(1).Text, "N")
        SQL = SQL & " and nomcalibrador2 = " & DBSet(txtAux(4).Text, "T")
    ...
    If b And (Modo = 3 Or Modo = 4) Then
        If txtAux(5).Text <> "" Then
            SQL = "select count(*) from rcalidad_calibrador where codvarie = " & DBSet(txtAux(0).Text, "N")
            SQL = SQL & " and codcalid <> " & DBSet(txtAux(1).Text, "N")
            SQL = SQL & " and nomcalibrador3 = " & DBSet(txtAux(5).Text, "T")
