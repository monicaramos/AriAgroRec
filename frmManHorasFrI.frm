...
Begin VB.Form frmManHorasFrI 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Entrada de Horas Frutas Inma"
   ClientHeight    =   10335
   ClientLeft      =   195
   ClientTop       =   180
   ClientWidth     =   19005
   Icon            =   "frmManHorasFrI.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   10335
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   10
      TabIndex        =   1
      Tag             =   "Campo|N|S|||horas|codcampo|00000000|S|"
      Top             =   4545
      Width           =   1170
   End
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmManHorasFrI.frx":000C
      Height          =   8145
      Left            =   90
      TabIndex        =   16
      Top             =   780
   ...
   End
End
Attribute VB_Name = "frmManHorasFrI"
...
'    txtAux2(6).visible = Not B
    ...
    '[Monica]23/05/2019
    txtAux2(6).visible = Not B
    txtAux2(12).visible = Not B
    txtAux2(13).visible = Not B
...
Private Sub BotonModificar()
    ...
    'Llamamos al form
    txtAux(0).Text = DataGrid1.Columns(6).Text ' codcapat
    txtAux2(0).Text = DataGrid1.Columns(7).Text ' nomcapat
    
    
    txtAux(6).Text = DataGrid1.Columns(2).Text 'codvarie
    txtAux2(6).Text = DataGrid1.Columns(3).Text 'nomvarie
    txtAux(1).Text = DataGrid1.Columns(0).Text 'fechahora
    txtAux(7).Text = DataGrid1.Columns(4).Text 'codtraba
    txtAux2(7).Text = DataGrid1.Columns(5).Text 'nomtraba
...
'    SelComboBool DataGrid1.Columns(2).Text, Combo1(0)
    ' *****************************************************
    txtAux(10).Text = DataGrid1.Columns(1).Text 'campo

    txtAux2(12).Text = DataGrid1.Columns(8).Text 'categoria
    txtAux2(11).Text = DataGrid1.Columns(9).Text ' nom categoria
    txtAux2(13).Text = DataGrid1.Columns(10).Text ' precio
    
    txtAux(2).Text = DataGrid1.Columns(12).Text 'importe
    txtAux(3).Text = DataGrid1.Columns(13).Text 'complemento
    txtAux(4).Text = DataGrid1.Columns(11).Text 'horas
...
Dim cad As String, Indicador As String
    ...
    cad = "codtraba = " & adodc1.Recordset.Fields(4) & " and  codvarie = " & adodc1.Recordset.Fields(2) & " and fechahora = " & DBSet(adodc1.Recordset.Fields(0), "F")
    cad = cad & " and codcampo = " & adodc1.Recordset.Fields(1)
    ' ***************************************
    CargaGrid CadB
...
'    chkVistaPrevia.Value = CheckValueLeer(Name)
    ...
    '****************** canviar la consulta *********************************+
    CadenaConsulta = "SELECT horas.fechahora, horas.codcampo, horas.codvarie, variedades.nomvarie, horas.codtraba, straba.nomtraba, "
    CadenaConsulta = CadenaConsulta & " horas.codcapat, rcapataz.nomcapat, "
    CadenaConsulta = CadenaConsulta & " straba.codcateg, salarios.nomcateg, salarios.impsalar, "
    CadenaConsulta = CadenaConsulta & " horas.horasdia, horas.importe, horas.compleme, horas.penaliza, "
    ...
    CadenaConsulta = CadenaConsulta & " FROM  horas, variedades, rcapataz, straba left join salarios on straba.codcateg = salarios.codcateg "
...
'    adodc1.ConnectionString = Conn
    ...
    tots = "S|txtAux(1)|T|Fecha|1400|;S|btnBuscar(0)|B||195|;S|txtAux(10)|T|Campo|1200|;S|btnBuscar(5)|B||195|;"
    tots = tots & "S|txtAux(6)|T|Variedad|1050|;S|btnBuscar(3)|B||195|;S|txtAux2(6)|T|Variedad|2000|;"
    tots = tots & "S|txtAux(7)|T|Codigo|1000|;S|btnBuscar(4)|B||195|;S|txtAux2(7)|T|Trabajador|3350|;"
    tots = tots & "S|txtAux(0)|T|Capataz|850|;S|btnBuscar(1)|B||195|;N|txtAux2(0)|T|Capataz|2000|;"
    tots = tots & "S|txtAux2(12)|T|Sal|600|;N|txtAux2(11)|T|Categoria|1400|;S|txtAux2(13)|T|Pr.H|600|;"
    tots = tots & "S|txtAux(4)|T|Horas|750|;"
    tots = tots & "S|txtAux(2)|T|Importe|1200|;"
    tots = tots & "S|txtAux(3)|T|Complem.|1100|;"
    tots = tots & "S|txtAux(9)|T|Penalización|1400|;"
    tots = tots & "S|txtAux(5)|T|F.Recibo|1400|;S|btnBuscar(2)|B||195|;N||||0|;S|chkAux(0)|CB|IC|340|;N|txtAux(8)|T|Almacen|800|;"
...
Private Sub CargaForaGrid()
    ...
    ' **********************************************************************
    If adodc1.Recordset.EOF Then Exit Sub
    txtAux2(0).Text = DataGrid1.Columns(7).Text
    txtAux2(11).Text = DataGrid1.Columns(9).Text
    txtAux2(8).Text = PartidaCampo(DataGrid1.Columns(1).Text)
End Sub
        ...
        Case 7 ' codigo de trabajador
            If PonerFormatoEntero(txtAux(Index)) Then
                ...
                If txtAux2(Index).Text = "" Then
                    ...
                    PonerFoco txtAux(Index)
                Else
                    SQL = "select salarios.codcateg, salarios.nomcateg, salarios.impsalar from salarios inner join straba on straba.codcateg = salarios.codcateg where codtraba = " & DBSet(txtAux(7).Text, "N")
                    Set Rs = New ADODB.Recordset
                    Rs.Open SQL, conn, adOpenForwardOnly, adLockPessimistic, adCmdText
                    If Not Rs.EOF Then
                        txtAux2(12).Text = DBLet(Rs.Fields(0).Value, "N")
                        txtAux2(11).Text = DBLet(Rs.Fields(1).Value, "N")
                        txtAux2(13).Text = DBLet(Rs.Fields(2).Value, "N")
                    End If
                    Set Rs = Nothing
                End If
            End If
...
Dim SQL As String
    ...
    SQL = SQL & " and codtraba = " & DBSet(txtAux(7).Text, "N")
    SQL = SQL & " and codvarie = " & DBSet(txtAux(6).Text, "N")
    SQL = SQL & " and codcampo = " & DBSet(txtAux(10).Text, "N")
...
Dim SQL As String
    ...
    SQL = "select sum(importe) importe , sum(coalesce(compleme,0)) compleme, sum(coalesce(penaliza,0)) penaliza, sum(coalesce(horasdia,0)) horas from (" & cadena & ") aaaaa"
    ...
    If Not Rs.EOF Then
        ...
        If Rs.Fields(3).Value <> 0 Then Horas = DBLet(Rs.Fields(3).Value, "N") 'Solo es para saber que hay registros que mostrar
    
        txtAux2(1).Text = Format(Importe, "###,###,##0.00")
        txtAux2(2).Text = Format(Compleme, "###,###,##0.00")
        txtAux2(3).Text = Format(Penaliza, "###,###,##0.00")
        txtAux2(4).Text = Format(Horas, "###,###,##0.00")
