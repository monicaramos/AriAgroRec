...
Begin VB.Form frmManHorasDestajo 
   ...
   Caption         =   "Entrada de Horas Destajo Trabajadores"
   ClientHeight    =   6120
   ClientLeft      =   45
   ClientTop       =   30
   ClientWidth     =   16080
   Icon            =   "frmManHorasDestajo.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6120
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   6
      TabIndex        =   2
      Tag             =   "Código Variedad|N|N|0|999999|horasdestajo|codvarie|000000|S|"
      Top             =   4920
      Width           =   465
   End
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   7
      TabIndex        =   4
      Tag             =   "Cajas|N|N|||horasdestajo|numcajon|#,##0.00||"
      Top             =   4950
      Width           =   885
   End
   Begin VB.CommandButton btnBuscar 
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   8
      TabIndex        =   7
      Tag             =   "Horas|N|N|||horasdestajo|horas|###,##0.00||"
      Top             =   4950
      Width           =   1170
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   8
      TabIndex        =   5
      Tag             =   "Kilos|N|S|||horasdestajo|kilos|###,##0||"
      Top             =   4950
      Width           =   900
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   8
      TabIndex        =   6
      Tag             =   "Importe|N|N|||horasdestajo|importe|###,##0.00||"
      Top             =   4950
      Width           =   990
   End
   Begin VB.TextBox txtAux2 
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   16
      TabIndex        =   3
      Tag             =   "Codigo Forfait|T|N|||horasdestajo|codforfait||S|"
      Top             =   4935
      Width           =   1155
   End
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   10
      TabIndex        =   1
      Tag             =   "Fecha|F|N|||horasdestajo|fechahora|dd/mm/yyyy|S|"
      Top             =   4905
      Width           =   1320
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   6
      TabIndex        =   0
      Tag             =   "Código|N|N|0|999999|horasdestajo|codtraba|000000|S|"
      Top             =   4905
      Width           =   800
   End
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmManHorasDestajo.frx":000C
      Height          =   4545
      Left            =   120
      TabIndex        =   12
      Top             =   765
   ...
   End
End
Attribute VB_Name = "frmManHorasDestajo"
...
Private WithEvents frmFor As frmComercial 'forfaits
...
Private Sub BotonBuscar()
    ...
    CargaGrid "horasdestajo.codtraba = -1"
    '*******************************************************************************
    'Buscar
    For I = 0 To txtAux.Count - 1
        txtAux(I).Text = ""
...
'    SelComboBool DataGrid1.Columns(2).Text, Combo1(0)
    ...
    txtAux2(2).Text = DataGrid1.Columns(6).Text
...
'    If Not SepuedeBorrar Then Exit Sub
    ...
    SQL = SQL & vbCrLf & "Variedad: " & adodc1.Recordset.Fields(3) & " " & adodc1.Recordset.Fields(4)
    SQL = SQL & vbCrLf & "Forfait: " & adodc1.Recordset.Fields(5) & " " & adodc1.Recordset.Fields(6)
 ...
 TerminaBloquear
    ...
    Select Case Index
       ...
       Case 1 ' Fecha
        ...
        Case 2 'codigo de forfait
            AbrirFrmManForfaits Index
...
'    chkVistaPrevia.Value = CheckValueLeer(Name)
    ...
    '****************** canviar la consulta *********************************+
    CadenaConsulta = "SELECT horasdestajo.codtraba, straba.nomtraba, horasdestajo.fechahora, horasdestajo.codvarie, variedades.nomvarie, "
    CadenaConsulta = CadenaConsulta & "horasdestajo.codforfait, forfaits.nomconfe, horasdestajo.numcajon, horasdestajo.kilos, horasdestajo.importe, "
    CadenaConsulta = CadenaConsulta & " horasdestajo.horas "
    CadenaConsulta = CadenaConsulta & " FROM horasdestajo, straba, variedades, forfaits "
    CadenaConsulta = CadenaConsulta & " WHERE horasdestajo.codtraba = straba.codtraba and "
    CadenaConsulta = CadenaConsulta & " horasdestajo.codvarie = variedades.codvarie and "
    CadenaConsulta = CadenaConsulta & " horasdestajo.codforfait = forfaits.codforfait "
    
    '************************************************************************
    
    CadB = ""
    CargaGrid "horasdestajo.codtraba is null"
...
Private Sub frmFor_DatoSeleccionado(CadenaSeleccion As String)
    txtAux(2).Text = RecuperaValor(CadenaSeleccion, 1) 'codigo forfait
    txtAux2(2).Text = RecuperaValor(CadenaSeleccion, 2) 'nombre forfait
End Sub
...
Private Sub frmTra_DatoSeleccionado(CadenaSeleccion As String)
    txtAux(0).Text = RecuperaValor(CadenaSeleccion, 1) 'codigo trabajador
    txtAux2(0).Text = RecuperaValor(CadenaSeleccion, 2) 'nombre trabajador
End Sub

Private Sub frmVar_DatoSeleccionado(CadenaSeleccion As String)
    txtAux(7).Text = RecuperaValor(CadenaSeleccion, 1) 'codigo variedad
    txtAux2(7).Text = RecuperaValor(CadenaSeleccion, 2) 'nombre variedad
End Sub
...
'    adodc1.ConnectionString = Conn
    ...
    End If
    If Ascendente Then
        SQL = SQL & " ORDER BY  horasdestajo.fechahora, horasdestajo.codtraba "
    Else
        '********************* canviar el ORDER BY *********************++
        SQL = SQL & " ORDER BY  horasdestajo.fechahora desc, horasdestajo.codtraba "
        '**************************************************************++
    End If
    ...
    tots = "S|txtAux(0)|T|Código|1000|;S|btnBuscar(0)|B||195|;S|txtAux2(0)|T|Nombre Trabajador|2500|;"
    tots = tots & "S|txtAux(1)|T|Fecha|1350|;S|btnBuscar(1)|B||195|;"
    tots = tots & "S|txtAux(7)|T|Código|1000|;S|btnBuscar(3)|B||195|;S|txtAux2(7)|T|Variedad|1700|;"
    tots = tots & "S|txtAux(2)|T|Forfait|1800|;S|btnBuscar(2)|B||195|;S|txtAux2(2)|T|Descripción|1500|;"
    tots = tots & "S|txtAux(6)|T|Cajones|1100|;"
    tots = tots & "S|txtAux(4)|T|Kilos|1000|;"
    tots = tots & "S|txtAux(3)|T|Importe|1200|;"
    tots = tots & "S|txtAux(5)|T|Horas |1100|;"
    ...
    Select Case Index
        Case 0 'codigo de trabajador
            If PonerFormatoEntero(txtAux(Index)) Then
                ...
                Else
                    If txtAux(6).Text <> "" Then RealizarCalculos
                End If
            End If
        ...
        Case 6 ' cajas
            ...
            If PonerFormatoDecimal(txtAux(6), 10) Then
        ...
        Case 4 ' kilos
            ...
            If PonerFormatoEntero(txtAux(4)) Then
                ' calculamos importe
                 txtAux(3).Text = Importe(txtAux(4).Text, txtAux(2).Text)
                 If PonerFormatoDecimal(txtAux(3), 3) Then
                    ' calculamos horas
                     txtAux(5).Text = Horas(txtAux(3).Text, txtAux(0).Text)
                     PonerFormatoDecimal txtAux(5), 3
                 End If
            End If
...
Dim Mens As String
    ...
    If Modo = 3 Then   'Estamos insertando
        SQL = "select count(*) from horasdestajo where codtraba = " & DBSet(txtAux(0).Text, "N")
        SQL = SQL & " and fechahora = " & DBSet(txtAux(1).Text, "F")
        SQL = SQL & " and codvarie = " & DBSet(txtAux(7).Text, "N")
        SQL = SQL & " and codforfait = " & DBSet(txtAux(2).Text, "T")
        
        If TotalRegistros(SQL) <> 0 Then
            MsgBox "El trabajador existe para esta fecha, variedad. Reintroduzca.", vbExclamation
            PonerFoco txtAux(0)
            B = False
        End If
    End If
    ...
    If KeyAscii = teclaBuscar Then
        If Modo = 1 Or Modo = 3 Or Modo = 4 Then
            Select Case Index
                ...
                Case 7: KEYBusqueda KeyAscii, 3 'variedad
                Case 2: KEYBusqueda KeyAscii, 2 'forfait
            End Select
        End If
    Else
        KEYpress KeyAscii
...
Private Sub RealizarCalculos()
    'calculamos kilos
     txtAux(4).Text = Kilos(txtAux(6).Text, txtAux(2).Text)
     If PonerFormatoEntero(txtAux(4)) Then
        ' calculamos importe
         txtAux(3).Text = Importe(txtAux(4).Text, txtAux(2).Text)
         If PonerFormatoDecimal(txtAux(3), 3) Then
