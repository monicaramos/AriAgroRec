...
Begin VB.Form frmFVARFacturas 
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.ComboBox Combo1 
         ...
         TabIndex        =   29
         Tag             =   "Tipo retencion|N|N|||fvarcabfact|tiporeten|||"
         Top             =   585
         Width           =   4560
      End
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Image imgFec 
         ...
         Index           =   2
         Left            =   6525
         Picture         =   "frmFVARFacturas.frx":0010
...
Private WithEvents frmFVarCL As frmBasico2
Attribute frmFVarCL.VB_VarHelpID = -1
...
'    DesplazamientoVisible Me.Toolbar1, btnPrimero, B, NumReg
    ...
    BloquearCmb Me.Combo1(2), Modo = 0 Or Modo = 2 Or Modo = 5
...
'    Check1(0).Enabled = ((Modo = 3 Or Modo = 4) And Combo1(0).Text <> "FVG") Or Modo = 1
    ...
    '[Monica]10/03/2017: Si la fra. está sin contabilizar o contabilizada pero sin descontar dejamos cambiar en donde descontarla
    Combo1(1).Enabled = ((Modo = 4 And (Check1(2).Value = 0)) Or Modo = 3 Or Modo = 1) And Text1(4).Text <> ""
    Text1(5).Enabled = (Modo = 4 Or Modo = 3 Or Modo = 1)
    If Modo = 4 Then
        ...
        If Check1(2).Value = 1 Then Check1(4).Enabled = False
    End If
...
Private Function CargarIban() As Boolean
Dim Sql As String
    
    On Error GoTo eCargarIban

    CargarIban = False

    Sql = "insert into tmpinformes (codusu, codigo1, nombre1, fecha1, importe1, nombre2) values (" & vUsu.Codigo & "," & DBSet(Text1(3), "N") & ","
    Sql = Sql & " " & DBSet(Text1(0).Text, "T") & "," & DBSet(Text1(2), "F") & "," & DBSet(Text1(1).Text, "N") & ","
    Sql = Sql & DBSet(DevuelveDesdeBDNew(cConta, "formapago", "iban", "codforpa", DBSet(Text1(25).Text, "N"), "N"), "T") & ")"
...
eCargarIban:
    MuestraError Err.Number, "Cargar Iban", Err.Description
End Function
...
'[Monica]20/08/2018: sustituido esto por lo de abajo pq introducimos el tipo de retencion
...
Dim IvaExento
    ...
    '[Monica]23/11/2017: En el caso de ser una factura a cliente
    '                    Ponemos el control de que si es un cliente intracomunitario, el iva del concepto ha de ser exento
    If Text1(29).Text <> "" Then
        Dim Intracom As Integer
        Dim SqlIntra As String
        
        Intracom = 0
        SqlIntra = "select intracom from paises where codpaise in (select codpaise from clientes where codclien = " & DBSet(Text1(29).Text, "N") & ")"
        Intracom = ComprobarCero(DevuelveValor(SqlIntra))
        If Intracom <> 0 Then
            Set vSeccion = New CSeccion
            If vSeccion.LeerDatos(Text1(3)) Then
                If vSeccion.AbrirConta Then
                    IvaExento = vSeccion.TipIvaExento
                End If
            End If
            If Not vSeccion Is Nothing Then
                vSeccion.CerrarConta
            ...
            If txtAux(8).Text <> "" Then
                If txtAux(8).Text <> IvaExento Then
                    If MsgBox("El cliente es intracomunitario, y el concepto no tiene asociado un iva exento. " & vbCrLf & "¿ Desea continuar ?" & vbCrLf, vbQuestion + vbYesNo + vbDefaultButton2) = vbNo Then b = False
                End If
            End If
        End If
    End If
